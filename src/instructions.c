...
RRR_INSTRUCTION as_rrr_instruction(uint32_t instruction) {
    ...
    if (d.rs2 > (1 << 7))
        FAIL("R_INSTRUCTION.rs2 decode error: size %d", d.rs2);
    if (d.rs3 > (1 << 7))
        FAIL("R_INSTRUCTION.rs3 decode error: size %d", d.rs3);
    return d;
}
...
RL_INSTRUCTION as_rl_instruction(uint32_t instruction) {
    RL_INSTRUCTION d = {
        .op0 = (
            // <TotalBits=4>
            // <Start=0> <End=3> <Length=4> <Offset=0> <Zero=false>
    ...
    if (d.op1 > (1 << 3))
        FAIL("I_INSTRUCTION.op1 decode error: size %d", d.op1);
    if (d.lsstype > (1 << 2))
        FAIL("I_INSTRUCTION.lsstype decode error: size %d", d.lsstype);
    if (d.lsdtype > (1 << 2))
        FAIL("I_INSTRUCTION.lsdtype decode error: size %d", d.lsdtype);
    return d;
}
...
RI_INSTRUCTION as_ri_instruction(uint32_t instruction) {
    RI_INSTRUCTION d = {
        ...
        .imm21 = (
            // <TotalBits=21>
            // <Start=11> <End=31> <Length=21> <Offset=0> <Zero=false>
            (get_word_bits(instruction, 11, 31) << 0)
            |
            // Unsigned
            0
        )
    ...
    if (d.op0 > (1 << 4))
        FAIL("U_INSTRUCTION.op0 decode error: size %d", d.op0);
    if (d.rd > (1 << 7))
        FAIL("U_INSTRUCTION.rd decode error: size %d", d.rd);
    if (d.imm21 > (1 << 21))
        FAIL("U_INSTRUCTION.imm21 decode error: size %d", d.imm21);
    return d;
}
...
bool is_setpc_instruction(const RI_INSTRUCTION* decoded_instruction) {
    if (decoded_instruction == NULL) FAIL("Received NULL pointer on is_setpc_instruction");
    if (decoded_instruction->op0 != 0b1001) return false;
    return true;
}
...
bool is_ldu_instruction(const RL_INSTRUCTION* decoded_instruction) {
    if (decoded_instruction == NULL) FAIL("Received NULL pointer on is_addi_instruction");
    if (decoded_instruction->op0 != 0b0101) return false;
    if (decoded_instruction->op1 != 0b000) return false;
    return true;
}
...
char* format_ldu_operation(RL_INSTRUCTION* decoded_instruction) {
    if (!is_ldu_instruction(decoded_instruction)) return NULL;
    sprintf(format_memory, "LDU <rd=%s> <rs1=%s> <imm11=%u>",
        register_to_name(decoded_instruction->rd),
        register_to_name(decoded_instruction->rs1),
        decoded_instruction->imm11
